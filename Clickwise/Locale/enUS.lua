...
-- Smart mode (Smart.lua)
L["%s is not in your spellbook"] = true
L["A set with that name exists."] = true
L["AND"] = true
L["Curse"] = true
L["Disease"] = true
L["I am in combat"] = true
L["IF"] = true
L["Magic"] = true
L["NOT"] = true
L["No frame shows %s."] = true
L["No smart sets yet: make one in the Smart tab."] = true
L["No such set."] = true
L["No such set: %s"] = true
L["Nothing"] = true
L["Nothing after this rule can be reached: it holds for every click."] = true
L["Nothing applies to this unit right now: the click does its default (or nothing in combat)."] = true
L["OR"] = true
L["Poison"] = true
L["Set %s for %s (used by: %s)"] = true
L["Smart"] = true
L["THEN"] = true
L["That name is too long (24 characters at most)."] = true
L["Type a name for the set."] = true
L["a condition this version does not know"] = true
L["a taunt aims at the enemy, so a test of the unit itself (dead) cannot share its rule"] = true
L["does not hold now: %s"] = true
L["dropped"] = true
L["needs a value: %s"] = true
L["no action"] = true
L["no binding"] = true
L["no spell named"] = true
L["none"] = true
L["none of its conditions holds now"] = true
L["nothing"] = true
L["number %d"] = true
L["otherwise: %s  ->  %s"] = true
L["otherwise: %s  ->  dropped: %s"] = true
L["otherwise: %s  ->  skipped: %s"] = true
L["skipped"] = true
L["the macro line would be over the length limit"] = true
L["you know no spell of that buff group"] = true
L["you know no taunt spell"] = true

-- Smart mode labels (Smart.lua)
L["I am in a raid"] = true
L["my form / stance is"] = true
L["my spec is"] = true
L["unit has a curable debuff"] = true
L["unit has buff"] = true
L["unit has debuff"] = true
L["unit has debuff type"] = true
L["unit has my buff"] = true
L["unit is dead"] = true
L["unit is me"] = true
L["unit's class is"] = true
L["unit's role is"] = true

-- Smart tab (ConfigSmart.lua, ConfigBindings.lua)
L["%s is used by a binding (%s): remove that binding first."] = true
L["(no condition)"] = true
L["(no sets yet)"] = true
L["A set holds at most %d rules."] = true
L["All of them"] = true
L["Any one of them"] = true
L["Auras are read when the click is written, so a rule that looks at them is skipped in combat."] = true
L["Conditions"] = true
L["Delete rule"] = true
L["Delete set"] = true
L["For your own frame (%d / %d characters):"] = true
L["Give the condition a value: %s"] = true
L["Make a rule set in the Smart tab first."] = true
L["Make or pick a rule set first."] = true
L["New rule"] = true
L["New set"] = true
L["No binding runs this set yet: pick 'Smart set' as the Action on the Bindings tab."] = true
L["Not in your spellbook - a rule for it is left out until you learn it."] = true
L["Nothing in this set applies to your own frame right now."] = true
L["Otherwise"] = true
L["Rename"] = true
L["Rule set"] = true
L["Rules (the first that holds wins)"] = true
L["Runs the rules of a set you build in the Smart tab: the first rule that holds decides what this click casts. If none holds, the click does nothing unless the set has an 'otherwise'."] = true
L["Save otherwise"] = true
L["Save rule"] = true
L["Smart set"] = true
L["Smart.lua was not loaded. Fully restart the game client (a /reload is not enough after files are added to the .toc)."] = true
L["Then"] = true
L["Type a name and press New set. Then add rules; a click runs the set once you pick 'Smart set' as its action on the Bindings tab."] = true
L["Type a spell name."] = true
L["Used when no rule above holds. 'Nothing' leaves the click to whatever else is on it."] = true
...
L["Move tooltip"] = true
L["Done moving"] = true
L["Reset tooltip"] = true
L["Drag this box to where the unit tooltip should appear. Right-click it (or press Done moving) when it is in place."] = true
