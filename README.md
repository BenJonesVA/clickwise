...
- **Smart mode.** Build a named rule set in the Smart tab and give a click the action "Smart set". A set is an ordered list of rules, **IF** up to four conditions (all of them, or any one) **THEN** a spell, a buff group or your taunt, plus an optional **otherwise**; the first rule that holds wins. Conditions are things the game can answer when you click (the unit is dead, you are in combat, your talent spec, your form or stance, you are in a raid), things about the unit that cannot change in a fight (its role, its class, whether it is you) and auras on the unit (a buff or debuff by name, a debuff type, a debuff you can remove). Auras are read when the click is set up, so a rule that looks at them is skipped in combat. So is **the unit's health is below 10-90%**: a macro cannot read health, so it is read when the click is written (out of combat, refreshed as health changes) and the editor warns you. A spell is picked from a searchable list of your spellbook, not typed, and the action **Run macro** picks one of your saved macros by name (its lines run through a hidden button of ours, so start its spells with `[@mouseover]` to act on the frame you click). **Groups** share a condition between rules: make a group on the left of the tab, give it conditions, and put rules in it with *In group*; a rule in a group needs the group's conditions too. Each frame gets its own compiled macro (`/cw smart [unit]` shows every rule and the result), which stops at 255 characters. Deleting a set a click still runs is refused.
